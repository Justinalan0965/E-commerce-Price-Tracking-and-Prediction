...
async def ecommerce(message: types.Message):
    ...
    if matchFound == "amzn":
        ...
        lastprice = product_price.replace(",","").replace("₹","")
        print("from main bot")
        print(lastprice)
        l = len(ratings)
...
async def untrackcmd(message: types.Message):
    await message.reply("<i>looking for the link in your messages...</i>",parse_mode="HTML")
    sno = message.text[-1:]
    list_size = flipsize()
    print(list_size)
    try:
        ...
        if new_n <= list_size:
            ...
            untrackF(new_n)
            await message.reply("<b>Okay😉</b>\n\nI will not track price for this product.\n\nYou can send any Flipkart product link.I will make sure to update you when the price drops!\n\n<i>Use /fliplist to see the list of products I'm tracking for you.</i>",parse_mode="HTML")
        else:
            print("Invalid Sno")
...
async def untrackcmd(message: types.Message):
    ...
    try:
        ...
        if new_n <= amlist_size:
            print("Entered the second if block")
            untrackA(new_n)
            await message.reply("<b>Okay😉</b>\n\nI will not track price for this product.\n\nYou can send any Amazon product link.I will make sure to update you when the price drops!\n\n<i>Use /amznlist to see the list of products I'm tracking for you.</i>",parse_mode="HTML")
        else:
            print("Invalid Sno")
...
async def callbackmethod(call: types.Message):
    ...
    if call.data == '2':
        ...
        pred_price,string = getThelink(pred_link,lastprice)
        await call.message.answer("<b>Please note that this price will not be exactly same as the actual price since it is predicted using the previous prices.</b>\n\n"+"In next month the price of this product is expected to be ₹"+str(float(pred_price))+"\n\n"+string+"\n",parse_mode="HTML")
